    ...
    while(*paecparams->pbcontinue)
    {
		...
		while (pQueueMic->size())
        {
            ...
            else
            {
				...
				if (bMatch)
				{
					//printf("[MATCH]------------------------------ idxMic = %d, idxRef=%d\n",idxMic,idxRef);
					bMatchedBefore = true;
					...
					{
						printf("[aecpthread] nSizeMic = %d, nSizeRef = %d\n",nSizeMic,nSizeRef);
						continue;
					}
					//printf("[aecpthread] --->nSizeMic = %d, nSizeRef = %d\n",nSizeMic,nSizeRef);
				...
				else
				{
					...
					SPRINTF_S(dbg_str,"Audio data match failed, ref queue = %d", pQueueRef->size());
					DEBUG_INFO(dbg_str);
#endif//PRINT_AEC_INFO
				}
			}//pQueueRef->size() >0
